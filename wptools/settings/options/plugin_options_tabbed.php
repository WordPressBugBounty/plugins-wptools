...
$mypage = new Page('Settings WPTools', array(
	'type' => 'submenu',
	'parent_slug' => 'options-general.php', // Defina o menu pai correto
	'menu_title' => 'Settings WPTools',
...
// die(var_export($wpmemory_memory ));
...
$msg2 .= '</big>';
$settings[remove_accents(esc_attr__('General Settings', 'wptools'))][esc_attr__('Instructions', 'wptools')] = array('info' => $msg2);
$fields = array();
...
if (empty($wptools_checkversion)) {
	$fields[] = array(
		...
		'label' => esc_attr__('Add Max Memory Limit', 'wptools') . '<br />Currently: ' . esc_attr($wpmemory_memory['wp_limit']),
...
if (empty($wptools_checkversion)) {
	$fields[] = array(
		...
		'label' => esc_attr__('Add Max Execution Time Limit', 'wptools') . '<br />Currently: ' . esc_attr($time_limit),
		'id' => 'rate_limiting', // (optional, will default to name)
		'value' => 'red', // (optional, will default to '')
		'select_options' => array(
...
if (empty($wptools_checkversion)) {
	$fields[] = array(
		...
		'label' => esc_attr__('Add Max Upload File size Limit', 'wptools') . '<br />Currently: ' . esc_attr($upload_limit),
		'id' => 'rate_limiting', // (optional, will default to name)
		'value' => 'red', // (optional, will default to '')
		'select_options' => array(
			array('value' => '', 'label' => esc_attr__('Select', "wptools")),
...
$fields[] = array(
	'type' 	=> 'radio',
	'name' 	=> 'wptools_improve_wp_dashboard_performance',
	'label' => esc_attr__('Improve Dashboard Performance by prevent WordPress from fetching news and Clean up WordPress admin dashboard.', "wptools"),
	'radio_options' => array(
		array('value' => 'yes', 'label' => esc_attr__('Yes', "wptools")),
		array('value' => 'no', 'label' => esc_attr__('No', "wptools")),
	)
...
//
$msg2 .= '<br />';
$msg2 .= '<b>' . esc_attr__('Show Server Load Average (CPU Usage) for the last minute at top admin bar.', 'wptools') . '</b>';
$msg2 .= '<br />';
...
$msg2 = '<b>' . esc_attr__('Show Disk Usage.', 'wptools') . '</b>';
$msg2 .= '<br />';
$msg2 .= esc_attr__('Use this option only in Linux Servers.', 'wptools');
$msg2 .= '<br />';
...
try {
	...
	//var_export($wpmemory_memory['wp_limit']);
	//die();
	...
	if ($perc > .7)
		echo '</span>';
	echo '|&nbsp;&nbsp;&nbsp;' . esc_attr__('Total Php Server Memory', 'wptools') . ': ' . esc_attr($wpmemory_memory['limit'], 'wptools') .
		' MB&nbsp;&nbsp;&nbsp;';
	if ($wptools_server_ram > 0) {
		echo '|&nbsp;&nbsp;&nbsp;' . esc_attr__('Total Hardware Memory', 'wptools') . ': ';
...
$msg2 = ob_get_contents();
ob_end_clean();
$settings[remove_accents(esc_attr__('Memory Usage', 'wptools'))][esc_attr__('Information', 'wptools')] = array('info' => $msg2);
...
if (filter_var($ip_server, FILTER_VALIDATE_IP)) {
	$msg2 = '<br />';
	$msg2 .= '<b>' . esc_attr__('Your Server IP Address:', 'wptools') . '&nbsp;&nbsp;' . $ip_server . '</b>';
	$msg2 .= '<br />';
} else {
	$msg2 = '<br />';
	$msg2 .= '<b>' . esc_attr__('Unable to get your server Ip. Probably blocked by your hosting company.', 'wptools') . '</b>';
	$msg2 .= '<br />';
}

$msg2 = '<br />';
$msg2 .= '<b>' . esc_attr__('WordPress will send email only when PHP fatal errors happens.', 'wptools') . '</b>';
$msg2 .= '<br />';
$msg2 .= '<b>' . esc_attr__('Do you want receive email alerts when notices/warnings and javascript errors happens? ', 'wptools') . '</b>';
$msg2 .= '<br />';
$msg2 .= '<b>' . esc_attr__('For the weekly email, the default setting is "Yes." Choose "No" if you prefer not to receive it.', 'wptools') . '</b>';
$msg2 .= '<br />';
$msg2 .= '<b>' . esc_attr__('Left Blank to use your default Wordpress email.Then, click save changes.', 'wptools');
$msg2 .= '<br />';
$msg2 .= '<br />';
...
$fields[] = array(
	...
	'label' => esc_attr__('Send email notification when notices/warnings or erros occurs? (limited at 1 email each 15 minutes)', 'wptools'),
	'radio_options' => array(
		array('value' => 'yes', 'label' => esc_attr__('yes', "wptools")),
		array('value' => 'no', 'label' => esc_attr__('no', "wptools"))
	)
