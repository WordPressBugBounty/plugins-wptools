...
$wptools_radio_server_load =  trim(sanitize_text_field(get_option('wptools_radio_server_load', 'no')));
...
$wptools_enable_reinstall = "no";

if (!empty($wptools_checkversion)) {
	$wptools_enable_reinstall = sanitize_text_field(get_option('wptools_enable_reinstall', 'no'));
}
...
if (is_admin()) {
	...
	*/
	...
	add_filter("plugin_action_links_$plugin", "wptools_add_action_links");
...
add_action('wp_loaded', 'wptools_load_chat');
